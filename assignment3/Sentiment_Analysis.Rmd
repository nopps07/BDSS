...
First of all, we extract the list of the AI researchers from the Wikipedia website. After cleaning the extracted data, we use the information in order to obtain the researchers' published articles via Scopus API. As noted in the code chunk below, you have to record your own Scopus API key in your local system to be able to utilize the service. 
...
**I don't know if this runs or not, translated code from python to R, but 8GB of RAM not enough to open the json in R on my pc**
...
Why these were higly positive or negative is not clear. When using the Bing and afinn lexicon we only have a match with 2 words, using the loughran lexicon only one word. The NRC lexicon has the most matches.
...
It is thus not clear if it will work well on research papers. Let's try it out.
**Note that computing sentiment scores using the VADER lexicon is very intensive**
...
The Sentiment WordNet is a lexical resource for opinion mining. It is built on top of WordNet, a lexical database for the English language. 
...
vader = social media (emotions) -> inappropriate for large texts
nrc is more useful in our case.
