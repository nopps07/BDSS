...
If we now take a look at the arxiv data, we can see that the amount of articles per year is much more balanced, which is more appropriate to conduct research. Besides, this dataset contains more than 10,000 articles **before and after the release of ChatGPT** and hence it should be able to answer the formulated research question more accurately.
...
```{r warning=FALSE, eval=TRUE, include=TRUE, cache=TRUE}
for(i in 1:nrow(scopus)){
...
By looking at one of the outliers the problem becomes clear. The spikes are caused by days on which only a few articles were published and these sentiment scores are either highly positive or negative. This is due to the fact that sometimes only a few words from the abstracts have a match with the lexicon, causing the overall sentiment score to be very high or low if those words are very positive or negative. 

Choosing the right lexicon is of crucial importance when performing sentiment analysis. For this reason we decided to try out different lexicons and see which one has the most word-matches with our abstracts.
Eventually the most suitable lexicon for our analysis seemed to be the `NRC` lexicon. When looking at the example below, the `Bing` and `Afinn` lexicons both only have a match with 2 words and using the `Loughran` lexicon only one word. The `NRC` lexicon is the clear winner compared to the rest with 17 matches. According to ChatGPT, recommended lexicons for analyzing research papers are: NRC, VADER, LIWC (pay-only) and SentiWordNet. Thus, we proceed with the `VADER`, `SentiWordNet` and `NRC`.
...
After having tried out the SentiWordNet, we decided not to include it in our analysis. The reason for this is that it is hard to work with in R and the objectivity score only depends on the positivity and negativity score. 
This means that we do not get any more information out of it compared to the other lexicons. 
...
The result indicates that the desired number of topics is 20, and hence we proceed with the analysis with this number. A question might arise the size of 14 may be a better choice than 20 since the convergence seems to begin at the point of 14. This size selection does not come with an absolute measure that allows us to precisely determine an optimal number. Also, the referred tutorial exhibits a similar pattern in its result and still proceeds with the size of 20. Hence, we do not intend to conduct additional investigations to justify the selected size. Now, we are able to build the Latent Dirichlet Allocation (LDA) model, which is trained on the DTM using the Gibbs sampling with 1000 iterations. Note that we do not intend to experiment with other sampling methods and different hyperparameters as it is not the primary focus of the analysis.

### 3.4.4. LDA model

**Note**: the code below is computationally very intensive. It might take more than an hour depending on the performance of your device.

```{r lda, cache=TRUE, include=TRUE}
# number of topics
K <- 20
...
Clearly, the results of Topic 14, 15, and 18 show that the sentiment score of the articles included in these topics has improved more notably than the rest since the birth of ChatGPT. The other topics seem to exhibit more neutral change over time, and specifically, Topic 10's sentiment score decreases although its magnitude does not seem significant. Hence, we confirm that ChatGPT influenced certain networks of AI more positively than the rest. If you closely pay attention to the probable keywords of Topic 14, they are related to 'language' and 'generation', which are highly correlated with the primary concept of ChatGPT. Similarly, the words in Topic 18, 'Feature' and 'classification', seem to be one of the prominent concepts of AI in recent times. Therefore, it is logical to make an inference that this AI topic benefits from the innovation, and researchers in this field express more positive ideas towards the idea.

# 4. Conclusion

The research started with the goal to answer the following question: "Did the sentiment of AI in academia change since the launch of ChatGPT?". In particular, we collected the abstracts of the published academic paper, talking about artificial intelligence, available in Scopus and Arxiv. Due to the size of the data, the arxiv acted as the main data source for further analyses. Specifically, we applied sentiment analysis in order to answer the primary research question. The result indicates that, according to the method and the data, the sentiment in AI has increased over time. The trend is more obvious in the recent period of time, and the statistical significance test supported that the birth of ChatGPT did boost the speed of the change. Besides, topic modelling discovered that the data can be divided into a group of communities, which display different patterns of sentiment change. The trendy topics exhibit a more sharply increasing pattern in their sentiments.

Nonetheless, it is crucial to pinpoint the limitations of our research design and propose future research approaches that could be taken. Despite the usefulness of sentiment analysis, its application is more suitable for general text data like social media comments, reviews, and emails. In other words, sentiment analysis is fundamentally designed to determine the emotional tone of the message, which could be lacking in abstracts of academic papers. Normally, abstract texts are more declarative with many technical jargon which does not entail detectable emotions. As noted at the beginning of the analysis, none of the common lexicons is fundamentally designed to be used for this specific type of text. Recently, many AI influencers are active on social media channels such as LinkedIn and Twitter, and the used texts in such channels likely contain less formal concepts of words which would eventually be appropriate for sentiment analysis.

Moreover, the research assumed that the articles in the data represent the general opinion of experts and hence the result can be generalized. However, more extensive design could be taken in order to answer, for instance, whether or not there is a difference between the nationalities of the authors, the journals of the papers and so on. Besides, given the recent speed of AI development and its influence on our daily life, a large-scale public survey specifically related to AI may be necessary sooner or later. A serious discussion of whether or not AI is regulated has been ongoing within and between nations, which must lead to sufficient and reliable data collection to mandate a proper policy (if needed). In such efforts, we may be more closely able to evaluate the role of AI experts.

AI technology is no longer a special resource that is only available to special communities. Anyone who has access to the internet could easily benefit from the technology, and its accessibility and number of use cases are exponentially increasing. In other words, its social impact should never be underestimated, and thus we must carefully and smartly deal with the negatives coming out of this trend. It is the right time to approach this in a more multidisciplinary way, and hence experts in diverse fields (AI, social science, etc) and policy makers should be included. In this sense, we believe that this research has shown how these two areas could be combined.
...
4. OpenAI. (2023). ChatGPT (April 20 version) [Large language model]. https://chat.openai.com/ (Helped us create the plotly graphs we needed for our analysis)