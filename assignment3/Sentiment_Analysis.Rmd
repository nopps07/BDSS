...
---
# Collecting and Analyzing Big Data for Social Science

## Assignment 3

This notebook is used for collaborative works.
...
Even though that the authors of the papers are expected to be _influential_, we conclude that the total data size is not sufficient to obtain meaningful results. Therefore, we follow one more data collection in order to enlarge the inventory. **EXPLAIN ARXIV HERE (VALENTIN)** 
...
# TODO Elesevier API is done in my local drive. I will move the codes here later.
# Steps

# Step 1
# extract the publication from every researcher included in "clean_names"
# some have published excessive amounts of paper, which would create bias caused by those authors
# Hence, we RANDOMLY select N (initially set as 30) from each researcher

# Step 2
# Extract Abstracts from DOI in Step 1
```
...
```{r}
library(readr)
arxiv <- read_csv("arxiv.csv", col_types = cols(update_date = col_date(format = "%Y-%m-%d")))
head(arxiv)
```
...
```{r}
arxiv <- read_csv("arxiv_sentiments.csv", col_types = cols(update_date = col_date(format = "%Y-%m-%d")))
#arxiv <- read_csv("arxiv_cleaned.csv", col_types = cols(update_date = col_date(format = "%Y-%m-%d")))
...
```{r}
library(readr)
df = readr::read_csv('arxiv_cleaned.csv')
